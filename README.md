...
**Retune the money and the safe havens.** `LADDER` lists the prize for each rung (bottom to top); `SAFE\\\\\\\_RUNGS` lists the guaranteed checkpoints by rung number:

```js
const LADDER = \\\\\\\[100, 200, 300, 500, 1000, 2000, 4000, 8000, 16000, 64000, 250000, 1000000];
const SAFE\\\\\\\_RUNGS = \\\\\\\[4, 8];
```

**Dial the drama.** Tweak the reveal pacing (in milliseconds):

```js
const PULSE\\\\\\\_MS        = 1300; // suspense before the answer is revealed
const CORRECT\\\\\\\_HOLD\\\\\\\_MS = 1600; // how long the green sits before climbing
const WRONG\\\\\\\_HOLD\\\\\\\_MS   = 2300; // how long the red/green sits before game over
```
